import { discoverFtpFiles, discoverFtpFilesRecursive, testFtpConnection } from '../src/services/ftpConnectionService.js';
import pg from 'pg';
const { Pool } = pg;
...
async function testDiscovery() {
  ...
  try {
    ...
    for (const db of res.rows) {
      ...
      console.log('First 3 recFiles:', recFiles.slice(0, 3));
    }
  } catch (err) {
