...
import { 
  WorkflowBundle, 
  WorkflowBundleScope, 
  WorkflowBundleStatus 
} from '../types.js';

export interface CreateWorkflowBundleInput {
  name: string;
  description?: string;
  version?: string;
...
export const workflowBundleService = {
   ...
   */
  async createBundle(input: CreateWorkflowBundleInput): Promise<WorkflowBundle> {
    ...
    const scope = (input.scope || 'TEAM').toUpperCase() as 'PERSONAL' | 'TEAM' | 'GLOBAL_ENTERPRISE';
  ...
  async proposePromotion(
    bundleId: string, 
    targetScope: WorkflowBundleScope, 
    makerId: string, 
    makerName: string
  ...
  async reviewPromotion(
    ...
    const newStatus: WorkflowBundleStatus = isApprove ? 'APPROVED' : 'REJECTED';
    ...
    try {
      ...
      if (isApprove) {
        ...
        await queryPg(
          `UPDATE database_validation_workflows 
           SET visibility = $1, is_public = $2, updated_at = NOW() 
           WHERE id = $3`,
          [visibility, isPublic, row.workflow_id]
        );
      }
   ...
   */
  async getBundleById(id: string): Promise<WorkflowBundle | null> {
    const sql = `
      SELECT wb.*, w.name as workflow_name, t.name as team_name
      FROM workflow_bundles wb
      LEFT JOIN database_validation_workflows w ON wb.workflow_id = w.id
  ...
  _mapRowToBundle(row: any, workflowName?: string, teamName?: string): WorkflowBundle {
    return {
      id: row.id,
      ...
      version: row.version,
      scope: row.scope as WorkflowBundleScope,
      ...
      status: row.status as WorkflowBundleStatus,
