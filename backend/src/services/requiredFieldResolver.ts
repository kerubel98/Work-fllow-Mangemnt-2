 ...
 */
export function resolveRequiredColumnsForStage(
  stage: ProcessingStage,
  rules: ValidationCheckStep[],
  existingExtraction?: QueryExtraction
): StageFieldRequirement {
  const columnMap = new Map<string, { usedByRuleIds: Set<string>; required: boolean; dataType?: string }>();
  ...
  const requiredColumns: QueryColumn[] = Array.from(columnMap.entries()).map(([col, meta]) => ({
    sourceColumn: col,
  ...
  if (existingExtraction) {
    const extractedColNames = new Set(existingExtraction.selectedColumns.map((c: QueryColumn) => c.sourceColumn.toLowerCase()));
    for (const col of requiredColumns) {
      if (col.required && !extractedColNames.has(col.sourceColumn.toLowerCase())) {
        missingFromExtraction.push(col.sourceColumn);
        warnings.push(
          `Rule(s) [${col.usedByRuleIds.join(', ')}] require column '${col.sourceColumn}', but it is missing from QueryExtraction for stage '${stage.name}'`
        );
      }
    }
  }
 ...
 */
export function resolveRequiredColumnsForWorkflow(
  workflow: DatabaseValidationWorkflow,
  extractions: QueryExtraction[] = []
): Record<string, StageFieldRequirement> {
  const result: Record<string, StageFieldRequirement> = {};
