...
import { workflowEngineSingleton, ValidationPayloadEnvelope, WorkflowJobResult } from './workflowEngineSingleton.js';
...
export const investigationOrchestratorService = {
  ...
  async executeUniversalWorkflow(envelope: ValidationPayloadEnvelope): Promise<WorkflowJobResult> {
    ...
    let totalPassed = 0;
    let totalFailed = 0;
    const finalEvaluatedRecords: any[] = [];
    ...
    for (const hit of cacheHits) {
      ...
      if (hit.cached.verdict === 'PASS') totalPassed++;
      else totalFailed++;
    }
    ...
    if (inFlight.length > 0) {
      ...
      for (let i = 0; i < inFlight.length; i++) {
        ...
        if (res.status === 'fulfilled' && res.value) {
          ...
          if (res.value._validation_status === 'PASS') totalPassed++;
          else totalFailed++;
        } else {
          finalEvaluatedRecords.push({
            ...item.record,
    ...
    const jobPromise = (async () => {
      ...
      for (let stageIdx = 0; stageIdx < stages.length; stageIdx++) {
            ...
            for (let rIdx = 0; rIdx < currentActiveRecords.length; rIdx++) {
              ...
              if (txKey) {
                workflowEngineSingleton.cacheResult(
                  ...
                  'PAUSED_DB_OFFLINE',
                  'PAUSED_DB_OFFLINE',
                  { circuitBreaker: 'OPEN', error: liveness.error }
                );
              }
            }
            break;
          ...
          try {
            ...
            if (discoveredKey) {
              ...
              if (chainRes.rows.length > 0) {
                effectiveQueryKeys = chainRes.rows.map((r: any) => String(r.key_val));
        ...
        const externalResult = await executeExternalChunkQuery(
          extraction,
          { chunkId: `chunk-${jobId}-01`, sequence: 1, transactionIds: effectiveQueryKeys },
        ...
        currentActiveRecords.forEach((r, idx) => {
          const rKeys = getRecordCandidateKeys(r, idx);
          const matchedRow = partitionRes.rows.find((pr: any) => rKeys.includes(String(pr.tx_key)));
   ...
   */
  async executeInvestigationTask(
    taskId: string,
    ...
    for (let i = 0; i < batches.length; i += maxConcurrency) {
      ...
      await Promise.all(
        windowBatches.map(async (batchPlan) => {
          ...
          try {
            ...
            for (const txId of batchPlan.transactionIds) {
              const invTx: InvestigationTransaction = {
                id: `itx-${taskId}-${txId}`,
                taskId,
                batchId,
                transactionId: txId,
            ...
            for (const chunk of batchPlan.queryChunks) {
              ...
              const externalResult = await executeExternalChunkQuery(
                extraction,
                chunk,
                options.inMemoryRecords || transactions
              );
              ...
              if (externalResult.success) {
                ...
                await reconciliationService.persistExternalMirrorBatch(
                  taskId,
                  extraction.id,
                  extraction.targetDbId || 'EXTERNAL_DB',
                  mirrorRecords
                );

                // 5. Execute set-based relational reconciliation in PostgreSQL
                const reconOutcome = await reconciliationService.reconcileBatchInDatabase(
                  taskId,
                  batchId,
                  extraction.id,
                  effectiveKeyField
