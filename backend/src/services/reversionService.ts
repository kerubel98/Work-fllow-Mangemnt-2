...
import { SolutionProcessType, TransactionReversionSnapshot } from '../types.js';
...
export const reversionService = {
  ...
  async createSnapshot(
  ...
  ): Promise<TransactionReversionSnapshot> {
    ...
    await pool.query(
      `INSERT INTO transaction_reversion_snapshots 
    ...
    const snapshot: TransactionReversionSnapshot = {
      id: snapId,
      taskId,
      transactionId,
      processType,
   ...
   */
  async revertTask(taskId: string, userId: string): Promise<{ success: boolean; revertedCount: number; restoredRows: any[] }> {
    const pool = getPostgresPool();

    // Fetch latest active snapshot
    const snapRes = await pool.query(
      `SELECT id, before_state, process_type
    ...
    const originalRows: any[] = typeof before_state === 'string' ? JSON.parse(before_state) : before_state;
    ...
    try {
      ...
      for (let i = 0; i < originalRows.length; i++) {
        ...
        await pool.query(
          ...
          [taskId, batchId, rowNum, JSON.stringify(canonical), JSON.stringify(raw)]
        );
      }
      ...
      await pool.query(
        `UPDATE issues
         ...
         WHERE id = $2;`,
        [JSON.stringify(canonicalRows), taskId]
      );

      // Mark snapshot as reverted
      await pool.query(
        `UPDATE transaction_reversion_snapshots
   ...
   */
  async revertTransaction(taskId: string, transactionId: string, userId: string): Promise<{ success: boolean; transactionId: string; restoredRow: any }> {
    const pool = getPostgresPool();

    // Find latest snapshot containing this transaction
    const snapRes = await pool.query(
      `SELECT id, before_state
       FROM transaction_reversion_snapshots
    ...
    try {
      ...
      await pool.query(
        `INSERT INTO transaction_reversion_snapshots 
         ...
         VALUES ($1, $2, $3, 'INTERNAL_STAGED_FIX', $4, 'Single-transaction granular rollback', $5, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, $5);`,
        [rowSnapId, taskId, transactionId, JSON.stringify([matchedRow]), userId]
      );
   ...
   */
  async getReversionHistory(taskId: string): Promise<TransactionReversionSnapshot[]> {
    const pool = getPostgresPool();
    const res = await pool.query(
      `SELECT id, task_id, transaction_id, process_type, before_state, after_state, reason, created_by, created_at, reverted_at, reverted_by
       FROM transaction_reversion_snapshots
       WHERE task_id = $1
       ORDER BY created_at DESC;`,
      [taskId]
    ...
    return res.rows.map(r => ({
      ...
      processType: r.process_type as SolutionProcessType,
