...
import { AssetType } from './assetSharingService.js';
...
export class AssetVerificationService {
   ...
   */
  async getAssetRecord(assetType: AssetType, assetId: string): Promise<any | null> {
    const pool = getPostgresPool();
    let table = '';
    if (assetType === 'VALIDATION_BOX') table = 'validation_boxes';
    else if (assetType === 'WORKFLOW') table = 'database_validation_workflows';
    else if (assetType === 'DB_CONFIG') table = 'database_table_mappings';
    else return null;
  ...
  async testAssetAgainstTransactions(params: {
    assetType: AssetType;
    assetId: string;
    checkerUser: { id: string; name: string };
    sampleLimit?: number;
    ...
    const { rows: txRows } = await pool.query(
      `SELECT id, task_id, row_number, raw_data, canonical_data 
    ...
    const sampleResults: Array<{ transactionId: string; verdict: 'PASS' | 'FAIL' | 'ERROR'; details?: string }> = [];
    ...
    for (const tx of txRows) {
      ...
      if (params.assetType === 'VALIDATION_BOX') {
        ...
        const missingKey = searchParams.find((p: string) => data[p] === undefined);
        if (missingKey) {
  ...
  async approveAsset(params: {
    ...
    const pool = getPostgresPool();
    let table = '';
    if (params.assetType === 'VALIDATION_BOX') table = 'validation_boxes';
    else if (params.assetType === 'WORKFLOW') table = 'database_validation_workflows';
    else if (params.assetType === 'DB_CONFIG') table = 'database_table_mappings';
  ...
  async declineAsset(params: {
    ...
    const { rows } = await pool.query(
      `UPDATE ${table} 
       ...
       WHERE id = $2
       RETURNING *;`,
      [params.feedback, params.assetId]
    );
  ...
  async verifyEditPermission(params: {
    assetType: AssetType;
    assetId: string;
    requestingUser: { id: string; name: string; role?: string };
  }): Promise<boolean> {
    const asset = await this.getAssetRecord(params.assetType, params.assetId);
    if (!asset) return false;

    // If not locked, any team member with write access or the maker can edit
    if (!asset.is_locked) return true;
    ...
    throw new Error(
      `Permission Denied: Asset '${asset.name || asset.id}' is locked. Only the approving Checker (@${asset.approved_by_user_name || 'Checker'}) can modify or unlock this approved version.`
    );
  }
