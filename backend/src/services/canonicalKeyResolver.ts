...
export interface KeyResolutionResult {
  ...
  sourcePrecedence: 'EXPLICIT_OVERRIDE' | 'TABLE_CONFIG_OVERRIDE' | 'VALIDATION_BOX_MAPPING' | 'DATASET_MATCH' | 'FALLBACK';
  isResolved: boolean;
  warning?: string;
}
 ...
 */
export function resolveCanonicalKey(options: KeyResolutionOptions): KeyResolutionResult {
  const {
    explicitKey,
    workflow,
    stage,
  ...
  if (Array.isArray(transactions) && transactions.length > 0) {
    ...
    const fallbackMatch = availableKeys.find(k =>
  ...
  if (strictFailFast) {
    throw new Error(
      `[CanonicalKeyResolver] Unable to determine primary correlation key for workflow '${workflow?.name || workflow?.id || 'unknown'}'. Please configure an explicit key in DB table mapping or validation box.`
    );
  }
 ...
 */
export function validateDatasetKeys(
  transactions: Record<string, any>[],
  primaryKey: string
): { isValid: boolean; missingCount: number; warning?: string } {
  if (!Array.isArray(transactions) || transactions.length === 0) {
