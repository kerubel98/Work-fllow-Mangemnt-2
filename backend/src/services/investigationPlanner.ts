...
import { resolveRequiredColumnsForWorkflow, StageFieldRequirement } from './requiredFieldResolver.js';
...
export function planInvestigation(
  ...
  const enabledStages = (workflow.stages || []).filter((s: ProcessingStage) => s.enabled);
  ...
  for (const [_, req] of Object.entries(stageFieldReqs) as [string, StageFieldRequirement][]) {
  ...
  // Build stage plans
  const stages: InvestigationStagePlan[] = enabledStages.map((stage: ProcessingStage) => {
    ...
    return {
      ...
      requiredColumns: req ? req.requiredColumns.map((c: QueryColumn) => c.sourceColumn) : [],
