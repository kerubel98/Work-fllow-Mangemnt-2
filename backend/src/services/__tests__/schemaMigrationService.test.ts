...
describe('schemaMigrationService', () => {
  ...
  it('successfully executes atomic addDirectoryField with physical column addition and audit', async () => {
    ...
    const added = await schemaMigrationService.addDirectoryField(field as any);
    expect(added.key).toBe(testFieldKey);

    // Verify physical column exists in database
    const colCheck = await pool.query(
      "SELECT column_name FROM information_schema.columns WHERE table_name = 'central_transaction_repository' AND column_name = $1;",
      [testFieldKey]
    );
  ...
  it('rolls back completely if a schema migration encounters an invalid DDL operation', async () => {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO global_standard_directory (id, field_name, display_name, data_type, is_required)
         VALUES ($1, $2, $3, $4, $5);`,
        [`gsd-${badKey}`, badKey, 'Bad Col', 'invalid_sql_type', false]
      );
      ...
      await pool.query(
        `INSERT INTO schema_version_audit (action, field_name, data_type, status, error_details)
         VALUES ('ADD_COLUMN', $1, 'NON_EXISTENT_TYPE', 'ROLLED_BACK', $2);`,
        [badKey, err.message]
      );
    } finally {
      client.release();
