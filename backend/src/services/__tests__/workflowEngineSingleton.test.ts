...
describe('workflowEngineSingleton', () => {
  ...
  it('partitions incoming transactions into fresh execution and cache hits', () => {
    ...
    const firstPartition = workflowEngineSingleton.partitionIncomingTransactions(
      workflowId,
      [tx1, tx2],
      'transaction_id'
    );
    ...
    const secondPartition = workflowEngineSingleton.partitionIncomingTransactions(
      workflowId,
      [tx1, tx2],
      'transaction_id'
    );
  ...
  it('handles in-flight promises avoiding duplicate execution across concurrent requests', async () => {
    ...
    const partition = workflowEngineSingleton.partitionIncomingTransactions(
      workflowId,
      [tx],
      'transaction_id'
    );
