...
import {
  buildQueryFromExtraction,
  resolveGroupedRows,
  MultiRowResolutionPolicy
} from '../externalDataQueryService.js';
import { QueryExtraction, QueryChunkPlan } from '../../types.js';
...
describe('Zero-Error Financial Reconciliation Pipeline: Hardening Suite (v2.1)', () => {
  ...
  describe('2. ANSI Identifier Quoting & Composite Tuples', () => {
    it('quotes composite column identifiers to prevent SQL injection and keyword collisions', () => {
      ...
      const { sql, parameters } = buildQueryFromExtraction(
        extraction,
        chunk,
        'cbs_transactions',
        'PostgreSQL',
    ...
    it('tags diagnostic-only metadata on records forwarded under REPORT', () => {
      ...
      activeRecords.forEach(r => {
        ...
        if (matched && matched._validation_status === 'FAIL' && matched._validation_action === 'REPORT') {
          (r as any)._isDiagnosticOnly = true;
          (r as any)._diagnosticTaintReason = matched._validation_details?.message;
        }
      });

      expect((activeRecords[0] as any)._isDiagnosticOnly).toBeUndefined();
      expect((activeRecords[1] as any)._isDiagnosticOnly).toBe(true);
      expect((activeRecords[1] as any)._diagnosticTaintReason).toBe('Fee mismatch');
    });
  });
});
