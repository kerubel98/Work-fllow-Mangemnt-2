...
export class StagingService {
   ...
   */
  async stageInboundEnvelope(envelope: MessageEnvelope): Promise<StagedMessage> {
    ...
    try {
      ...
      const existingRes = await client.query(
        `SELECT id, status, channel, created_issue_id FROM staged_messages WHERE dedupe_key = $1 LIMIT 1;`,
        [envelope.dedupeKey]
      );
      ...
      if (existingRes.rows.length > 0) {
        ...
        return this.getStagedMessageById(row.id) as Promise<StagedMessage>;
      }
      ...
      const parsedEnrichment = await attachmentParserService.parseAttachments(
      ...
      const initialStatus: StagedMessageStatus = 
        parsedEnrichment.confidenceScore >= 0.7 ? 'READY_FOR_TASK_CREATION' : 'NEW';
      ...
      for (const att of parsedEnrichment.attachments) {
        await client.query(
          `INSERT INTO staged_message_attachments (
  ...
  async getStagedMessages(params: {
    ...
    for (const r of res.rows) {
      const attRes = await pool.query(
        `SELECT * FROM staged_message_attachments WHERE staged_message_id = $1 ORDER BY created_at ASC;`,
        [r.id]
      );
   ...
   */
  async getStagedMessageById(id: string): Promise<StagedMessage | null> {
    const pool = getPostgresPool();
  ...
  async proposeTaskConversion(
    ...
    if (!staged) throw new Error(`Staged message ${id} not found.`);
    ...
    const updated = await pool.query(
      `UPDATE staged_messages
       ...
       WHERE id = $6
       RETURNING *;`,
      [maker.id, payload.notes || `Proposed by ${maker.name}`, payload.teamId || null, payload.priority || null, payload.title || null, id]
    );
   ...
   */
  async approveAndConvertTask(
    id: string,
    checker: { id: string; name: string },
    reviewNotes?: string
  ): Promise<{ staged: StagedMessage; issue: Issue }> {
    const pool = getPostgresPool();
    const staged = await this.getStagedMessageById(id);
    if (!staged) throw new Error(`Staged message ${id} not found.`);
    ...
    await pool.query(
      `UPDATE staged_messages
       SET status = 'CONVERTED_TO_TASK',
           ...
           updated_at = NOW()
       WHERE id = $4;`,
      [createdIssue.id, checker.id, reviewNotes || `Approved by ${checker.name}`, id]
    );
   ...
   */
  async rejectStagedMessage(id: string, actor: { id: string; name: string }, reason: string): Promise<StagedMessage> {
    const pool = getPostgresPool();
    await pool.query(
      `UPDATE staged_messages
       SET status = 'REJECTED',
           ...
           updated_at = NOW()
       WHERE id = $3;`,
      [reason || 'Rejected by operator', actor.id, id]
    );
    ...
    await pool.query(
      `UPDATE staged_messages
       SET team_id = $1,
           ...
           updated_at = NOW()
       WHERE id = $3;`,
      [targetTeamId, `Escalated to ${targetTeamId} by ${actor.name}: ${reason}`, id]
    );
   ...
   */
  async getExternalRequestSummary(teamId?: string): Promise<ExternalRequestSummary> {
    const pool = getPostgresPool();
    ...
    const channelRes = await pool.query(
      `SELECT channel, COUNT(*) as count FROM staged_messages ${teamFilter} GROUP BY channel;`,
      params
    );

    const providersRes = await pool.query(
      `SELECT COUNT(*) as count FROM provider_connections WHERE status = 'ACTIVE' ${teamId ? 'AND team_id = $1' : ''};`,
      params
    );

    const row = statsRes.rows[0] || {};
    const channelBreakdown: Record<SupportedMessageChannel, number> = {
      email: 0,
      teams: 0,
      whatsapp: 0,
      telegram: 0
    ...
    for (const ch of channelRes.rows) {
      if (channelBreakdown[ch.channel as SupportedMessageChannel] !== undefined) {
        channelBreakdown[ch.channel as SupportedMessageChannel] = parseInt(ch.count, 10);
      }
    }
   ...
   */
  async getProviderConnections(teamId?: string, userId?: string): Promise<ProviderConnection[]> {
    const pool = getPostgresPool();
    const conditions: string[] = [];
    const values: any[] = [];
    let idx = 1;
    ...
    const res = await pool.query(
      `SELECT * FROM provider_connections ${whereClause} ORDER BY created_at DESC;`,
      values
    );
  ...
  async saveProviderConnection(data: Partial<ProviderConnection>): Promise<ProviderConnection> {
    const pool = getPostgresPool();
  ...
  async testProviderConnection(id: string): Promise<{ success: boolean; message: string; details?: any }> {
    const pool = getPostgresPool();
    const res = await pool.query(`SELECT * FROM provider_connections WHERE id = $1;`, [id]);
    if (res.rows.length === 0) throw new Error(`Provider connection ${id} not found.`);
    ...
    await pool.query(
      `UPDATE provider_connections
       ...
       WHERE id = $3;`,
      [testResult.success ? 'ACTIVE' : 'ERROR', testResult.success ? null : testResult.message, id]
    );

    return testResult;
  }

  async fetchProviderMessages(id: string): Promise<{ stagedCount: number; job: ProviderFetchJob }> {
    const pool = getPostgresPool();
    const res = await pool.query(`SELECT * FROM provider_connections WHERE id = $1;`, [id]);
    if (res.rows.length === 0) throw new Error(`Provider connection ${id} not found.`);
    ...
    await pool.query(
      `INSERT INTO provider_fetch_jobs (id, provider_id, channel, status, started_at)
       VALUES ($1, $2, $3, 'RUNNING', NOW());`,
      [jobId, id, provider.channel]
    );
    ...
    try {
      ...
      for (const raw of rawMessages) {
        ...
        if (provider.team_id) envelope.teamId = provider.team_id;
        if (provider.user_id) envelope.personId = provider.user_id;
      ...
      await pool.query(
        `UPDATE provider_fetch_jobs
         ...
         WHERE id = $3;`,
        [messagesFound, messagesStaged, jobId]
      );
    } catch (err: any) {
  ...
  private mapStagedRow(r: any, attachments: any[] = []): StagedMessage {
    return {
      id: r.id,
      channel: r.channel,
