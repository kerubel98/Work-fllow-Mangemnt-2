...
export interface SchemaMigrationAuditEntry {
  id: number;
  version?: string;
  action: string;
  fieldName: string;
...
function mapTypeToPostgres(dataType: string): string {
...
export const schemaMigrationService = {
   ...
   */
  async initAndSyncCentralSchema(): Promise<{ addedCount: number; totalColumns: number }> {
   ...
   */
  async addDirectoryField(field: GlobalStandardDirectoryRecord): Promise<GlobalStandardDirectoryRecord> {
    const pool = getPostgresPool();
    const client = await pool.connect();
    ...
    try {
      ...
      await client.query(
        `INSERT INTO schema_version_audit (action, field_name, data_type, status)
         VALUES ('ADD_COLUMN', $1, $2, 'SUCCESS');`,
        [colName, pgType]
      );
      ...
      try {
        await pool.query(
          `INSERT INTO schema_version_audit (action, field_name, data_type, status, error_details)
           VALUES ('ADD_COLUMN', $1, $2, 'ROLLED_BACK', $3);`,
          [colName, pgType, err.message]
        );
      } catch (auditErr) {
   ...
   */
  async updateDirectoryField(
    id: string,
    updates: Partial<GlobalStandardDirectoryRecord>
  ): Promise<GlobalStandardDirectoryRecord> {
    const pool = getPostgresPool();
    const client = await pool.connect();
    ...
    try {
      ...
      const existingRes = await client.query(
        'SELECT * FROM global_standard_directory WHERE id = $1 OR field_name = $1 LIMIT 1;',
        [id]
      );
      ...
      if (oldColName !== newColName) {
        await client.query(
          `ALTER TABLE central_transaction_repository RENAME COLUMN "${oldColName}" TO "${newColName}";`
        );
      }
      ...
      if (updates.dataType && updates.dataType !== existing.data_type) {
        await client.query(
          `ALTER TABLE central_transaction_repository ALTER COLUMN "${newColName}" TYPE ${newPgType} USING "${newColName}"::${newPgType};`
        );
      }

      // Update global_standard_directory table
      const updatedField: GlobalStandardDirectoryRecord = {
        id: existing.id,
        key: newKey,
      ...
      await client.query(
        `UPDATE global_standard_directory SET
      ...
      await client.query(
        `INSERT INTO schema_version_audit (action, field_name, data_type, status)
         VALUES ('ALTER_COLUMN', $1, $2, 'SUCCESS');`,
        [newColName, newPgType]
      );
      ...
      try {
        await pool.query(
          `INSERT INTO schema_version_audit (action, field_name, data_type, status, error_details)
           VALUES ('ALTER_COLUMN', $1, 'UNKNOWN', 'ROLLED_BACK', $2);`,
          [id, err.message]
        );
      } catch (auditErr) {
        console.error('[SchemaMigration] Could not log audit failure:', auditErr);
   ...
   */
  async deleteDirectoryField(id: string): Promise<boolean> {
    const pool = getPostgresPool();
    const client = await pool.connect();
    ...
    try {
      ...
      await client.query(
        `INSERT INTO schema_version_audit (action, field_name, data_type, status)
         VALUES ('DELETE_FIELD', $1, $2, 'SUCCESS');`,
        [colName, existing.data_type || 'TEXT']
      );
   ...
   */
  async getAuditLogs(limit = 50): Promise<SchemaMigrationAuditEntry[]> {
    const pool = getPostgresPool();
    const { rows } = await pool.query(
      'SELECT * FROM schema_version_audit ORDER BY created_at DESC LIMIT $1;',
      [limit]
    );
    return rows.map(r => ({
      id: r.id,
