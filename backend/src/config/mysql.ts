...
export async function ensureDocumentTable(): Promise<void> {
  ...
  for (const collection of MYSQL_COLLECTIONS) {
    ...
    `);
    await getMySqlPool().execute(
      `INSERT IGNORE INTO \`${collection}\` (document_id, document, created_at, updated_at)
       SELECT document_id, document, created_at, updated_at
       FROM app_documents WHERE collection_name = ?`,
      [collection]
    );
  }
}
...
export async function getMySqlStatus() {
  ...
  try {
    ...
    const [tables] = await connection.query<any[]>(
      `SELECT table_name AS name, table_rows AS approximate_rows
