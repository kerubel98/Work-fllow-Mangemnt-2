  ...
  if (dbId) {
    ...
    if (dbRecord) {
      if (!connectionString && dbRecord.connectionString) {
        ...
        if (dbRecord.databaseName || dbRecord.database_name) targetDbName = dbRecord.databaseName || dbRecord.database_name;
        if (dbRecord.username) targetUser = dbRecord.username;
      }
    }
  }
  ...
  try {
    ...
    if (db.type === 'FTP' || db.type === 'SFTP') {
      try {
        ...
        dbConfigs.forEach(c => {
          if (c.stagingTableName) combined.push(c.stagingTableName);
          if (c.fileNamePattern) combined.push(c.fileNamePattern);
        });
      } catch {}
    }
    ...
    try {
      ...
      const mirrorRes = await queryPg(
        `SELECT table_name FROM information_schema.tables 
         WHERE table_schema = 'public' AND (table_name LIKE $1 OR table_name LIKE 'mirror_ftp_%')`,
        [`${safeDbPrefix}_%`]
      );
...
databaseRouter.get('/databases/:id/tables/:tableName/preview', async (req: Request, res: Response) => {
  try {
    const db = await repo.getDatabaseById(req.params.id);
    if (!db) return res.status(404).json({ error: 'Database connection not found' });
    const tableName = req.params.tableName;
    ...
    if (tableName.startsWith('mirror_')) {
      try {
        ...
        if (mirrorQueryRes.rows && mirrorQueryRes.rows.length > 0) {
          ...
          const colRes = await queryPg(
            `SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = $1 ORDER BY ordinal_position`,
            [tableName.toLowerCase()]
          );
          cleanCols = colRes.rows.map((r: any) => r.column_name).filter((c: string) => !sysCols.has(c));
        }
...
databaseRouter.get('/column-configurations/:id', async (req: Request, res: Response) => {
  try {
    const config = await repo.getColumnConfigurationById(req.params.id);
    if (!config) return res.status(404).json({ error: 'Column configuration not found' });
...
databaseRouter.post('/column-configurations', async (req: Request, res: Response) => {
  try {
    const {
      name,
      dbId,
...
databaseRouter.put('/column-configurations/:id', async (req: Request, res: Response) => {
  try {
...
databaseRouter.post('/column-configurations/test', async (req: Request, res: Response) => {
  try {
    const { config, sampleRows } = req.body;
    if (!config) return res.status(400).json({ error: 'Rule configuration is required' });

    let rows: Record<string, any>[] = Array.isArray(sampleRows) ? sampleRows : [];
    ...
    if (rows.length === 0 && config.dbId && config.tableName) {
      ...
      if (db) {
        try {
          ...
          if (liveRes && liveRes.rows) rows = liveRes.rows;
        } catch {
          ...
          try {
            ...
            if (mRes && mRes.rows) rows = mRes.rows;
          } catch {}
        }
      }
    }
    ...
    const priorityCols: RuleColumnPriority[] = Array.isArray(config.columns) ? config.columns : [];
    ...
    const formatColVal = (val: any, colCfg?: RuleColumnPriority) => {
      if (val === null || val === undefined) return '__NULL__';
      let str = String(val);
      if (colCfg?.matchMode === 'TRIMMED') str = str.trim();
      if (colCfg?.matchMode === 'CASE_INSENSITIVE') str = str.trim().toLowerCase();
      if (colCfg?.transform === 'LOWERCASE') str = str.toLowerCase();
      if (colCfg?.transform === 'UPPERCASE') str = str.toUpperCase();
      if (colCfg?.transform === 'DIGITS_ONLY') str = str.replace(/\D/g, '');
      return str;
    };
    ...
    const evaluateTypeConditions = (row: any, conditions: any[]): boolean => {
      ...
      return conditions.every(cond => {
        ...
        switch (cond.operator) {
          ...
          case 'NOT_IN': {
            const disallowed = condValStr.split(',').map((s: string) => s.trim().toLowerCase());
    ...
    const findMatchingTypeGroup = (groupRows: any[], typeGroups: any[]): any | null => {
      if (!Array.isArray(typeGroups) || typeGroups.length === 0) return null;
      for (const tg of typeGroups) {
        if (!Array.isArray(tg.conditions) || tg.conditions.length === 0) continue;
        const matched = groupRows.some(row => evaluateTypeConditions(row, tg.conditions));
        if (matched) return tg;
      }
      return null;
    };

    // Helper to evaluate expected leg / row count operator
    const evaluateExpectedCount = (actualCount: number, expected?: { operator: string; value: number }): boolean => {
      if (!expected || expected.value === undefined) return true;
      const target = Number(expected.value);
      const op = expected.operator;
      if (op === '==' || op === '=') return actualCount === target;
      if (op === '!=') return actualCount !== target;
      if (op === '>') return actualCount > target;
      if (op === '>=') return actualCount >= target;
      if (op === '<') return actualCount < target;
      if (op === '<=') return actualCount <= target;
    ...
    if (ruleType === 'DUPLICATE_CHECK' || ruleType === 'UNIQUE_CONSTRAINT') {
      ...
      const countRule = (Array.isArray(config.aggregationRules) && config.aggregationRules.find((a: any) => a.function === 'COUNT'))
      ...
      const groups = new Map<string, number[]>();
      rows.forEach((row, idx) => {
        const key = groupCols.map((col: string) => `${col}=${getVal(row, col) ?? ''}`).join(' | ');
      ...
      const defaultAggs: RuleAggregation[] = Array.isArray(config.aggregationRules) ? config.aggregationRules : [];
      const roleCol = config.roleColumn || sortedCols.find(c => c.role === 'DISCRIMINATOR')?.columnName;
      const globalSemanticRoles: SemanticRowRoleConfig[] = Array.isArray(config.semanticRoles) ? config.semanticRoles : [];
      const globalCrossRules: SemanticCrossRowRule[] = Array.isArray(config.crossRowRules) ? config.crossRowRules : [];
      ...
      groups.forEach((indices, groupKey) => {
        ...
        if (typeGroups.length > 0 && !matchedType) {
          indices.forEach(idx => {
            violations.push({
              rowIndex: idx,
              rowData: rows[idx],
              reason: `Type Group classification failed [${groupKey}]: Group records do not match any configured Type Group conditions (${typeGroups.map((t: any) => t.groupName).join(', ')}).`,
              matchedPriorityValues: {
                groupKey,
                rowCount: indices.length,
                configuredTypeGroups: typeGroups.map((t: any) => t.groupName)
              }
            });
          });
        ...
        if (rulesToEvaluate.length > 0) {
          const resolveRowRole = (row: any): string => {
            if (!roleCol) return '__UNASSIGNED__';
            const rawRoleVal = getVal(row, roleCol);
            if (rawRoleVal === null || rawRoleVal === undefined) return '__UNASSIGNED__';
            const strVal = String(rawRoleVal).trim().toLowerCase();

            for (const sr of rolesToUse) {
              const vals = sr.matchValues || (sr as any).matchingValues;
              if (Array.isArray(vals)) {
                const matches = vals.some((v: any) => String(v).trim().toLowerCase() === strVal);
                if (matches) return sr.roleName;
              }
            }
        ...
        aggsToRun.forEach((agg: any) => {
          ...
          if (agg.function === 'COUNT') {
            ...
            if (agg.function === 'SUM') computedVal = vals.reduce((a, b) => a + b, 0);
            else if (agg.function === 'AVG') computedVal = vals.length ? (vals.reduce((a, b) => a + b, 0) / vals.length) : 0;
            else if (agg.function === 'MIN') computedVal = Math.min(...vals);
            else if (agg.function === 'MAX') computedVal = Math.max(...vals);
          }
          ...
          if (op === '=' || op === '==') satisfied = (computedVal === target);
          else if (op === '!=') satisfied = (computedVal !== target);
          else if (op === '>') satisfied = (computedVal > target);
          else if (op === '>=') satisfied = (computedVal >= target);
          else if (op === '<') satisfied = (computedVal < target);
          else if (op === '<=') satisfied = (computedVal <= target);
          else satisfied = (computedVal === target);
      ...
      const txGroups = new Map<string, { rows: any[]; indices: number[] }>();
      rows.forEach((row, idx) => {
      ...
      const mappings: ColumnValueLabelMapping[] = Array.isArray(config.valueLabels) ? config.valueLabels : [];
      ...
      // Build lookup map
      const labelMap = new Map<string, ColumnValueLabelMapping>();
      mappings.forEach(m => {
...
databaseRouter.post('/ftp-staging/run-scheduled', async (req: Request, res: Response) => {
  try {
...
databaseRouter.get('/ftp-staging/unconfigured-folders', async (req: Request, res: Response) => {
  try {
...
databaseRouter.post('/ftp-staging/resolve-unconfigured-folder', async (req: Request, res: Response) => {
  try {
    const { folderPath, connectionId } = req.body;
    if (!folderPath || !connectionId) return res.status(400).json({ error: 'folderPath and connectionId are required' });
...
databaseRouter.get('/databases/:id/tables/:tableName/preview', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const formattedColumns = (queryResult.columns || []).map((col: any) => {
      ...
      try {
        ...
        if (mirrorResult.rows && mirrorResult.rows.length > 0) {
          ...
          const cleanRows = mirrorResult.rows.map(r => {
            ...
            for (const c of cleanCols) clean[c] = r[c];
            return clean;
          });
