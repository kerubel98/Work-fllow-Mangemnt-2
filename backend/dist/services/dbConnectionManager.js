...
export async function discoverTablesForDb(db) {
    ...
    if (db.type === 'MySQL') {
        ...
        try {
            ...
            let tables = [];
            if (targetDb) {
                const [rows] = await connection.query(`SELECT table_name AS name FROM information_schema.tables WHERE table_schema = ? AND table_type = 'BASE TABLE' ORDER BY table_name`, [targetDb]);
                tables = rows;
            }
            else {
...
export async function getTableColumnsForDb(db, tableName) {
    ...
    if (db.type === 'MySQL') {
        ...
        try {
            const targetDb = config.database;
            const [rows] = await connection.query(`SELECT column_name AS name, data_type AS type, is_nullable AS nullable, column_key AS col_key
