...
function resolveDbConfig(db) {
    ...
    if (db.connectionString) {
        try {
            ...
            if (parsedUrl.hostname && parsedUrl.hostname !== 'host')
 ...
 */
export async function testExternalDbConnection(db) {
    const start = Date.now();
    const config = resolveDbConfig(db);
    try {
        if (db.type === 'MySQL') {
            const pool = getExternalMysqlPool(`${db.id}:${config.host}:${config.port}:${config.database}`, config);
 ...
 */
export async function discoverTablesForDb(db) {
    const config = resolveDbConfig(db);
    if (db.type === 'Oracle') {
    ...
    if (db.type === 'MySQL') {
        ...
        let tables = [];
        if (targetDb) {
            const [rows] = await pool.query(`SELECT table_name AS name FROM information_schema.tables WHERE table_schema = ? AND table_type = 'BASE TABLE' ORDER BY table_name`, [targetDb]);
            tables = rows;
        }
        else {
            const [rows] = await pool.query(`SELECT table_name AS name FROM information_schema.tables WHERE table_schema = DATABASE() AND table_type = 'BASE TABLE' ORDER BY table_name`);
            tables = rows;
        }
        return tables.map((t) => String(t.name || Object.values(t)[0]));
    }
    if (db.type === 'PostgreSQL') {
        const client = new pg.Client({
            host: config.host,
...
export async function getTableColumnsForDb(db, tableName) {
    ...
    if (db.type === 'MySQL') {
        const pool = getExternalMysqlPool(`${db.id}:${config.host}:${config.port}:${config.database}`, config);
        const targetDb = config.database;
        const [rows] = await pool.query(`SELECT column_name AS name, data_type AS type, is_nullable AS nullable, column_key AS col_key
       FROM information_schema.columns
       WHERE table_schema = ${targetDb ? '?' : 'DATABASE()'} AND table_name = ?
       ORDER BY ordinal_position`, targetDb ? [targetDb, tableName] : [tableName]);
        return rows.map((r) => ({
            name: r.name,
            type: String(r.type).toUpperCase(),
            nullable: r.nullable === 'YES',
    ...
    if (db.type === 'PostgreSQL') {
        ...
        await client.connect();
        try {
            const res = await client.query(`SELECT c.column_name AS name, c.data_type AS type, c.is_nullable AS nullable,
...
export async function executeLiveQueryOnDb(db, query, params) {
    ...
    if (db.type === 'MySQL') {
        ...
        if (Array.isArray(result)) {
            const rows = result;
            const columns = fields?.map(f => f.name) || (rows.length > 0 ? Object.keys(rows[0]) : []);
            return {
                columns,
                rows,
