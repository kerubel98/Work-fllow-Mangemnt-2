...
describe('Workflow Bundles and Governance Segregation Refactor Test Suite', () => {
    ...
    it('2. Submits bundle promotion proposal to PENDING_CHECKER_REVIEW', async () => {
        ...
        const proposed = await workflowBundleService.proposePromotion(bundle.id, 'GLOBAL_ENTERPRISE', 'maker_operator_02', 'Maker Operator');
    ...
    it('3. Strictly rejects promotion self-approval (Anti-Self-Approval / Rule 7)', async () => {
        ...
        await workflowBundleService.proposePromotion(bundle.id, 'GLOBAL_ENTERPRISE', 'author_maker_03', 'Author Maker');
        // Attempt to approve by the same user who made the proposal
        await expect(workflowBundleService.reviewPromotion(bundle.id, 'author_maker_03', // Same as maker!
        'Author Maker', 'APPROVE')).rejects.toThrow(/Anti-Self-Approval Violation/);
    });
    it('4. Successfully approves bundle promotion when authorized by an independent Checker', async () => {
        ...
        await workflowBundleService.proposePromotion(bundle.id, 'GLOBAL_ENTERPRISE', 'maker_engineer_04', 'Maker Engineer');
        // Independent Checker review
        const approved = await workflowBundleService.reviewPromotion(bundle.id, 'checker_supervisor_99', 'Independent Checker Supervisor', 'APPROVE', 'Audit verified and ready for enterprise deployment.');
    ...
    it('5. Successfully records rejection feedback on denied bundle promotion', async () => {
        ...
        await workflowBundleService.proposePromotion(bundle.id, 'GLOBAL_ENTERPRISE', 'maker_junior_05', 'Junior Maker');
        const rejected = await workflowBundleService.reviewPromotion(bundle.id, 'checker_supervisor_99', 'Independent Checker Supervisor', 'REJECT', 'Missing currency tolerance rules in validation box.');
