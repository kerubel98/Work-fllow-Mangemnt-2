...
describe('Task Workflow Executions Lookup & Cross-Task Duplicates', () => {
    ...
    describe('PostgreSQL Cross-Task Duplicate Detection & Resolution Stored Procedures', () => {
        ...
        it('detects cross-task duplicate transactions via sp_detect_and_flag_cross_task_duplicates', async () => {
            ...
            await pool.query("INSERT INTO central_transaction_repository (" +
                ...
                "ON CONFLICT (transaction_key) DO UPDATE SET canonical_data = EXCLUDED.canonical_data", [taskA + '_' + dupTxnId, taskA, 'BATCH-001', JSON.stringify([taskA]), JSON.stringify({ transaction_id: dupTxnId, amount: 500 })]);
            // Ingest same transaction_id under Task B
            await pool.query("INSERT INTO central_transaction_repository (" +
                "transaction_key, task_id, original_task_id, current_task_id, batch_id, all_task_ids, is_duplicate, duplicate_count, canonical_data, raw_data" +
                ") VALUES ($1, $2, $2, $2, $3, $4, false, 1, $5, $5) " +
                "ON CONFLICT (transaction_key) DO UPDATE SET canonical_data = EXCLUDED.canonical_data", [taskB + '_' + dupTxnId, taskB, 'BATCH-001', JSON.stringify([taskB]), JSON.stringify({ transaction_id: dupTxnId, amount: 500 })]);
            ...
            const checkA = await pool.query('SELECT is_duplicate, duplicate_count, all_task_ids FROM central_transaction_repository WHERE transaction_key = $1', [taskA + '_' + dupTxnId]);
        ...
        it('moves transaction exclusively to target task via sp_move_transaction_to_task', async () => {
            ...
            const checkAfter = await pool.query('SELECT task_id, is_duplicate, duplicate_count, all_task_ids, duplicate_status FROM central_transaction_repository WHERE transaction_key = $1', [keyA]);
        ...
        it('removes transaction from task via sp_remove_transaction_from_task', async () => {
            ...
            const checkAfter = await pool.query('SELECT all_task_ids, duplicate_status FROM central_transaction_repository WHERE transaction_key = $1', [keyB]);
