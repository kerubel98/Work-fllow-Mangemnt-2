 ...
 */
export function getRowValueCaseInsensitive(row, colName) {
    if (!row || !colName)
        return undefined;
 ...
 */
function quoteIdentifier(col, dialect = 'PostgreSQL') {
    const clean = col.replace(/[`"]/g, '');
    return dialect === 'MySQL' ? `\`${clean}\`` : `"${clean}"`;
}
export function buildQueryFromExtraction(extraction, chunk, targetTable = extraction.targetDataSource || 'target_data', dialect = 'PostgreSQL', inMemoryRecords) {
    const parameters = [];
    let pIndex = 1;
    const nextPlaceholder = (val) => {
        parameters.push(val);
    ...
    const columns = extraction.selectedColumns && extraction.selectedColumns.length > 0
        ? extraction.selectedColumns.map((c) => quoteIdentifier(c.sourceColumn, dialect)).join(', ')
        : '*';
 ...
 */
export function resolveGroupedRows(groupedRows, policy = 'COMPOSITE_BUNDLE', groupConfig, aggregateSumColumns, sortColumn) {
    const resolved = {};
 ...
 */
export async function executeExternalChunkQuery(extraction, chunk, inMemoryRecords, options = {}) {
    ...
    try {
        ...
        const overrideKeyField = options.mappingConfig?.reconciliationKeyField || extraction.reconciliationKeyField;
        const allMappings = extraction.keyMappings && extraction.keyMappings.length > 0
            ? extraction.keyMappings
            : [{ inputField: 'transaction_id', sourceField: 'transaction_id', required: true }];
        ...
        if (targetDb && targetDb.status === 'online') {
            ...
            for (const row of allRows) {
                ...
                if (matchingInputRecord) {
                    ...
                    if (matchingInputRecord._rowNumber)
                        candidateKeys.push(`ROW-${matchingInputRecord._rowNumber}`);
                    if (matchingInputRecord.row_number)
                        candidateKeys.push(`ROW-${matchingInputRecord.row_number}`);
                    const tuple = allMappings
                        .map(m => String(getRowValueCaseInsensitive(matchingInputRecord, m.inputField) ?? '').trim())
                        .filter(Boolean)
                        .join(':::');
                    if (tuple)
                        candidateKeys.push(tuple);
                }
                for (const m of allMappings) {
            ...
            const correlated = resolveGroupedRows(groupedRows, options.multiRowPolicy, options.groupConfig, options.aggregateSumColumns, options.sortColumn);
            return {
                success: true,
        ...
        const requestedColNames = new Set((extraction.selectedColumns || []).map((c) => c.sourceColumn.toLowerCase()));
