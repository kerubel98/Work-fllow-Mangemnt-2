...
export class AttachmentParserService {
     ...
     */
    async parseAttachments(attachments = [], messageTextBody = '', messageSubject = '') {
        const results = [];
        const combinedTextParts = [messageSubject, messageTextBody];
        let extractedTabularRows = [];
        let detectedHeaders = [];
        ...
        const { category, urgency, parsedFields, confidenceScore } = this.extractFieldsFromCorpus(fullExtractedText, messageSubject, attachments.length, extractedTabularRows.length);
     ...
     */
    extractFieldsFromCorpus(text, subject, attachmentCount, rowCount) {
