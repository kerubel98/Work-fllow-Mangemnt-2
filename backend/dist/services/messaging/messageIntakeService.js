...
export class MessageIntakeService {
    ...
    async processEnvelope(envelope) {
        ...
        try {
            ...
            const existingRes = await client.query(`SELECT id, status, intent, linked_issue_id FROM incoming_messages WHERE dedupe_key = $1;`, [envelope.dedupeKey]);
            ...
            const { resolvedUserId, resolvedTeamId } = await this.resolveIdentities(client, envelope.sourceChannel, envelope.senderAddress);
      ...
      `;
            ...
            if (envelope.attachments && envelope.attachments.length > 0) {
                for (const att of envelope.attachments) {
                    await client.query(`INSERT INTO message_attachments (
     ...
     */
    async resolveIdentities(client, channel, address) {
        const cleanAddr = address.trim().toLowerCase();
        // Check personal identities
        const personalRes = await client.query(`SELECT user_id FROM personal_channel_configurations 
       WHERE channel = $1 AND LOWER(address) = $2 AND is_active = true 
       LIMIT 1;`, [channel, cleanAddr]);
        let resolvedUserId = personalRes.rows[0]?.user_id;
        // Check team identities
        const teamRes = await client.query(`SELECT team_id FROM team_channel_configurations 
       WHERE channel = $1 AND LOWER(address) = $2 AND is_active = true 
       LIMIT 1;`, [channel, cleanAddr]);
        let resolvedTeamId = teamRes.rows[0]?.team_id;
     ...
     */
    async resolveLinkedIssue(client, envelope) {
     ...
     */
    async executeIntentAction(envelope, intent, linkedIssueId, resolvedUserId, resolvedTeamId) {
        let createdIssueId;
        let replyText = '';
     ...
     */
    async getTeamConfigurations(teamId) {
        const pool = getPostgresPool();
        const res = await pool.query(`SELECT * FROM team_channel_configurations WHERE team_id = $1 ORDER BY created_at DESC;`, [teamId]);
    ...
    async saveTeamConfiguration(config) {
        const pool = getPostgresPool();
    ...
    async getPersonalConfigurations(userId) {
        const pool = getPostgresPool();
        const res = await pool.query(`SELECT * FROM personal_channel_configurations WHERE user_id = $1 ORDER BY created_at DESC;`, [userId]);
        return res.rows.map(r => ({
            id: r.id,
            userId: r.user_id,
            channel: r.channel,
    ...
    async savePersonalConfiguration(config) {
        const pool = getPostgresPool();
    ...
    async getRecentIncomingMessages(limit = 50, teamId, issueId) {
        const pool = getPostgresPool();
