...
export function computeMessageHash(text) {
    const normalized = (text || '').trim().toLowerCase().replace(/\s+/g, ' ');
    return crypto.createHash('md5').update(normalized).digest('hex').substring(0, 16);
}
export function detectContentType(filename, mimeType) {
...
export class EmailAdapter {
    ...
    verifySignature(payload, signatureHeader, secret) {
        if (!secret || !signatureHeader)
            return true; // Optional secret check
        try {
    ...
    normalize(raw) {
        ...
        const attachments = (raw.attachments || []).map((att, idx) => {
