    ...
    if (db.connectionString) {
        try {
            ...
            if (parsed.pathname && parsed.pathname !== '/')
                baseDirectory = parsed.pathname;
            if (parsed.protocol === 'ftps:' || parsed.protocol === 'sftp:')
                secure = true;
        }
        catch {
 ...
 */
async function testSocketPing(host, port, timeoutMs = 3000) {
 ...
 */
function parseColumnsFromRawData(filename, rawText) {
