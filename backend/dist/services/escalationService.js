...
export const escalationService = {
     ...
     */
    async getEscalationsForIssue(issueId) {
        if (!isPostgresConnected)
            return [];
        const res = await queryPg(`SELECT * FROM escalation_events WHERE issue_id = $1 ORDER BY created_at DESC`, [issueId]);
     ...
     */
    async executeActionCommand(input) {
        ...
        if (input.idempotencyKey) {
            const existing = await queryPg(`SELECT * FROM escalation_events WHERE idempotency_key = $1 LIMIT 1`, [input.idempotencyKey]);
        ...
        if (input.actionType === 'ESCALATE_ISSUE') {
            ...
            await queryPg(`UPDATE issues 
         SET escalated_to_team_id = $1, 
             escalation_reason = $2, 
             escalated_at = NOW(), 
             status = 'IN_PROGRESS' 
         WHERE id = $3`, [targetTeam, input.reason || 'Escalated via Incident Room', input.issueId]);
            ...
            await queryPg(`UPDATE escalation_events 
         SET status = 'CLOSED', resolved_at = NOW() 
         WHERE issue_id = $1 AND status != 'CLOSED'`, [input.issueId]);
            outputMessage = `Escalation room for ${input.issueId} closed.`;
        }
        // 3. Persist Escalation Event in PostgreSQL
        const insertRes = await queryPg(`INSERT INTO escalation_events (
     ...
     */
    async transitionStatus(eventId, newStatus) {
        const isResolved = newStatus === 'RESOLVED' || newStatus === 'CLOSED';
        const res = await queryPg(`UPDATE escalation_events 
       SET status = $1, 
           resolved_at = CASE WHEN $2 = true THEN NOW() ELSE resolved_at END 
       WHERE id = $3 
       RETURNING *`, [newStatus, isResolved, eventId]);
