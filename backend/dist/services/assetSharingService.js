...
export class AssetSharingService {
    ...
    async shareAsset(params) {
        ...
        if (params.targetType === 'TEAM') {
            ...
            if (params.assetType === 'VALIDATION_BOX')
                tableName = 'validation_boxes';
            else if (params.assetType === 'WORKFLOW')
                tableName = 'database_validation_workflows';
            else if (params.assetType === 'DB_CONFIG')
                tableName = 'database_table_mappings';
            if (tableName) {
                await pool.query(`UPDATE ${tableName} 
           SET team_id = $1, status = 'PENDING_CHECKER_TEST', updated_at = NOW() 
           WHERE id = $2;`, [params.targetId, params.assetId]);
            }
        }
        // Persist share audit log
        const { rows } = await pool.query(`INSERT INTO asset_shares (
    ...
    async adoptAsset(params) {
        ...
        if (params.assetType === 'VALIDATION_BOX') {
            ...
            const { rows } = await pool.query(`INSERT INTO validation_boxes (
        ...
        if (params.assetType === 'WORKFLOW') {
            ...
            const newName = params.customName || `${orig.name} (My Copy)`;
            const { rows } = await pool.query(`INSERT INTO database_validation_workflows (
        ...
        if (params.assetType === 'DB_CONFIG') {
            ...
            const { rows } = await pool.query(`INSERT INTO database_table_mappings (
     ...
     */
    async getTeamStagedAssets(teamId) {
        const pool = getPostgresPool();
        const [vboxesRes, wfRes, dbRes] = await Promise.all([
            pool.query(`SELECT * FROM validation_boxes 
         WHERE team_id = $1 
         ORDER BY created_at DESC;`, [teamId]),
            pool.query(`SELECT * FROM database_validation_workflows 
         WHERE team_id = $1 
         ORDER BY created_at DESC;`, [teamId]),
            pool.query(`SELECT * FROM database_table_mappings 
         WHERE team_id = $1 
         ORDER BY updated_at DESC;`, [teamId])
        ]);
        const formatVBox = (r) => ({
            id: r.id,
            name: r.name,
     ...
     */
    async getDirectSharesForUser(userId) {
        const pool = getPostgresPool();
        const { rows } = await pool.query(`SELECT * FROM asset_shares 
