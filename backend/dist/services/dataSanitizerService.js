 ...
 */
export function cleanAmount(val) {
 ...
 */
export function cleanBoolean(val) {
    if (val === null || val === undefined || val === '')
        return null;
    ...
    if (typeof val === 'string') {
        ...
        if (['true', '1', 't', 'y', 'yes', 'on'].includes(s))
            return true;
        if (['false', '0', 'f', 'n', 'no', 'off'].includes(s))
            return false;
    }
    return null;
}
...
export const dataSanitizerService = {
     ...
     */
    async getRequiredDirectoryFields() {
        try {
            const allFields = await repo.getGlobalStandardDirectory();
     ...
     */
    async autoMapHeaders(rawHeaders, standardFields) {
        const mapping = {};
        ...
        for (const rawH of rawHeaders) {
            ...
            if (/^(fe_)?utrnno$/i.test(slugH) || /^(txn|trans|transaction|tx)(id|no|num|ref|reference|key)?$/i.test(slugH) || /^(rrn|retrieval_ref_num|refnum|reference_no|ref_no|ref|refno)$/i.test(slugH) || slugH.includes('ref')) {
                ...
                if (target) {
                    mapping[cleanHeader] = target.key;
                    continue;
                }
            }
     ...
     */
    async validateMapping(fileMapping, sampleRow) {
     ...
     */
    async sanitizeRows(rows, fileMapping) {
        ...
        const effectiveMapping = { ...autoMapped, ...(fileMapping || {}) };
