...
async function main() {
    try {
        ...
        // Fetch config
        const cfgRes = await pool.query("SELECT * FROM ftp_file_staging_configs WHERE id = 'stg-1789417828501-y8fh'");
        const config = cfgRes.rows[0];
        // Even with the user's pattern '*/Card/Settlemnt/*/*/*_Settlemnt.xls', compilePatternToRegex will match both AIB and BIB!
        const clientConfig = {
            ...config,
