...
async function hydrateBoxConfigurations(box) {
    if (!box)
        return box;
...
validationBoxesRouter.put('/:id', async (req, res) => {
    try {
        const { name, description, boxType, category, targetDbId, targetTable, searchParameters, checkStep, columnConfigurationIds, columnConfigurations, matchKeyInput, matchKeyExternal, multiRowPolicy, groupConfig, dualSourceCondition, outputColumns, statusBinding, messageTemplate } = req.body;
        let mirrorTableName = undefined;
        if ((boxType === 'INGESTION_SEARCH' || boxType === 'RECONCILIATION') && targetDbId && targetTable) {
            const db = await repo.getDatabaseById(targetDbId);
            if (db) {
...
validationBoxesRouter.post('/test', async (req, res) => {
    try {
        const { box, sampleRecord, record, transaction, parameters, inputValues, params } = req.body;
        if (!box) {
        ...
        const buildParametersFromBox = (b) => {
            const rec = {};
            if (b.boxType === 'INGESTION_SEARCH' && Array.isArray(b.searchParameters)) {
                for (const sp of b.searchParameters) {
                    ...
                    if (fieldKey) {
                        rec[fieldKey] = sp.defaultValue || (fieldKey.toLowerCase().includes('amount') ? 149.99 :
                ...
                if (b.groupConfig?.groupIdField)
                    rec[b.groupConfig.groupIdField] = 'GRP-001';
                if (b.groupConfig?.eventPhaseField)
                    rec[b.groupConfig.eventPhaseField] = 'FINANCIAL_REQ';
            }
            else if (b.boxType === 'REPORT' && Array.isArray(b.outputColumns)) {
                for (const col of b.outputColumns) {
                    if (col.field)
                        rec[col.field] = 'SAMPLE_DATA';
                }
            }
        ...
        if (box.boxType === 'INGESTION_SEARCH') {
            ...
            const requiredParams = searchParams.filter((p) => p.required !== false);
            ...
            for (const param of searchParams) {
                ...
                if (val !== undefined && val !== null && String(val).trim() !== '') {
                    ...
                    if (param.inputField)
                        handledCols.add(String(param.inputField).toLowerCase());
                }
            }
            ...
            try {
                ...
                if (mapping?.columns && Array.isArray(mapping.columns)) {
                    tableColNames = mapping.columns.map((c) => typeof c === 'string' ? c : (c.columnName || c.name || c.key)).filter(Boolean);
                }
            }
            catch {
            ...
            for (const [key, rawVal] of Object.entries(testRecord)) {
                if (rawVal === undefined || rawVal === null || String(rawVal).trim() === '')
                    continue;
                if (handledCols.has(key.toLowerCase()))
                    continue;
            ...
            const comparator = (op === 'EQUALS' ? '=' : (op === 'NOT_EQUALS' ? '!=' : (op === 'NUMERIC_TOLERANCE' ? '=' : op)));
            ...
            if (isDual) {
                if (dsc?.sourceA?.field)
                    requiredParams.push(dsc.sourceA.field);
                if (dsc?.sourceB?.field)
                    requiredParams.push(dsc.sourceB.field);
            }
            else {
                if (sourceField)
                    requiredParams.push(sourceField);
            }
            if (Array.isArray(step.requiredParams)) {
                for (const p of step.requiredParams) {
                    if (p && !requiredParams.includes(p))
            ...
            if (Array.isArray(box.searchParameters)) {
                for (const sp of box.searchParameters) {
                    if (sp.required !== false) {
                        ...
                        if (k && !requiredParams.includes(k))
                            requiredParams.push(k);
                    }
                }
            }
