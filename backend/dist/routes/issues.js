        ...
        if (!effectiveTeamId && issueData.creatorId) {
            ...
            if (creator) {
                if (creator.permanentTeamId) {
                    ...
                    const permTeam = teams.find(t => t.teamType === 'permanent' && (t.managerId === creator.id || (t.memberIds && t.memberIds.includes(creator.id))));
                    if (permTeam) {
...
issuesRouter.patch('/:id/visibility', async (req, res) => {
    try {
...
issuesRouter.post('/:id/chat/accept-solution', async (req, res) => {
    try {
        ...
        if (acceptedProposal.processType === 'CAUTIOUS_PROCESS') {
            try {
                ...
                await reversionService.createSnapshot(issue.id, 'CAUTIOUS_PROCESS', acceptedBy, `Snapshot on cautious solution acceptance (${acceptedProposal.id})`);
            }
            catch (snapErr) {
...
issuesRouter.post('/:id/revert', async (req, res) => {
    try {
...
issuesRouter.get('/:id/reversions', async (req, res) => {
    try {
        const { reversionService } = await import('../services/reversionService.js');
...
issuesRouter.post('/:id/execute-script', async (req, res) => {
    try {
        ...
        try {
            const { reversionService } = await import('../services/reversionService.js');
            await reversionService.createSnapshot(issue.id, issue.processType || 'INTERNAL_STAGED_FIX', req.body.executedBy || issue.creatorId, 'Auto-snapshot before script execution');
        }
        catch (snapErr) {
...
issuesRouter.get('/:id/audit-dossier', async (req, res) => {
    try {
        const issue = await repo.getIssueById(req.params.id);
        if (!issue)
            return res.status(404).json({ error: 'Issue not found' });
        ...
        const [reversions, currentDatasetRes, taskExecutions] = await Promise.all([
            ...
            repo.getTaskWorkflowExecutions ? repo.getTaskWorkflowExecutions(issue.id).catch(() => ({ executions: [] })) : Promise.resolve({ executions: [] })
        ]);
        ...
        const dossier = {
            ...
            validationExecutions: taskExecutions.executions || [],
