...
transactionSettingsRouter.post('/settings/schema', async (req, res) => {
...
transactionSettingsRouter.get('/schema/config', async (_req, res) => {
...
transactionSettingsRouter.post('/schema/config', async (req, res) => {
    ...
    const history = Array.isArray(versionHistory) ? [...versionHistory] : (store.globalSchema.versionHistory || []);
    if (description) {
        history.unshift({
            version: newVersion,
            timestamp: now,
    ...
    store.globalStandardDirectory = standardFields.map((f) => ({
        id: f.id || `gsd-${f.key}`,
        key: f.key,
        label: f.label || f.key,
...
transactionSettingsRouter.get('/table-mappings', async (_req, res) => {
    if (isPostgresConnected) {
        try {
...
transactionSettingsRouter.post('/table-mappings', async (req, res) => {
    ...
    const mappedKeys = new Set((mappingObj.columns || [])
        .filter((c) => c.globalKey && c.physicalColumn && c.physicalColumn.trim() !== '' && c.globalKey !== 'unmapped')
        .map((c) => c.globalKey));
...
transactionSettingsRouter.get('/table-mappings/validate/:dbId/:tableName', async (req, res) => {
    ...
    const mappedKeys = new Set(mapping.columns
        .filter((c) => c.globalKey && c.physicalColumn && c.physicalColumn.trim() !== '' && c.globalKey !== 'unmapped')
        .map((c) => c.globalKey));
    const missingRequired = requiredKeys.filter((k) => !mappedKeys.has(k));
    const isComplete = missingRequired.length === 0;
...
transactionSettingsRouter.post('/settings/templates', async (req, res) => {
...
transactionSettingsRouter.delete('/settings/templates/:id', async (req, res) => {
    const { id } = req.params;
...
transactionSettingsRouter.post('/upload', async (req, res) => {
...
transactionSettingsRouter.get('/working-db', async (req, res) => {
    ...
    if (search) {
        ...
        results = results.filter(r => r.mappedData.transaction_id.toLowerCase().includes(q) ||
...
transactionSettingsRouter.delete('/working-db/clear', async (req, res) => {
...
transactionSettingsRouter.post('/workspace-table', async (req, res) => {
...
transactionSettingsRouter.post('/directory', async (req, res) => {
...
transactionSettingsRouter.delete('/directory/:id', async (req, res) => {
    const { id } = req.params;
    ...
    if (store.globalSchema && Array.isArray(store.globalSchema.standardFields)) {
        store.globalSchema.standardFields = store.globalSchema.standardFields.filter((r) => r.id !== id && r.key !== id);
    }
    if (isPostgresConnected) {
        try {
...
transactionSettingsRouter.delete('/directory', async (_req, res) => {
