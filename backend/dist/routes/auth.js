...
authRouter.post('/register', async (req, res) => {
    try {
        ...
        const existing = users.some(u => u.username.toLowerCase() === username.toLowerCase() || u.email.toLowerCase() === email.toLowerCase());
        if (existing) {
 ...
 */
authRouter.put('/users/:id/approve', async (req, res) => {
    try {
        const { id } = req.params;
...
authRouter.put('/users/:id/role', async (req, res) => {
    try {
        ...
        if (role)
            updates.role = role;
        if (canExecuteSelect !== undefined)
            updates.canExecuteSelect = canExecuteSelect;
        if (canExecuteUpdate !== undefined)
            updates.canExecuteUpdate = canExecuteUpdate;
 ...
 */
authRouter.put('/users/:id/workspace-sharing', async (req, res) => {
    try {
        const { id } = req.params;
