...
workspaceSettingsProposalsRouter.get('/', async (req, res) => {
    try {
        const teamId = req.query.teamId;
        const status = req.query.status;
        const escalatedTeamId = req.query.escalatedTeamId;
...
workspaceSettingsProposalsRouter.get('/:id', async (req, res) => {
    try {
...
workspaceSettingsProposalsRouter.post('/', async (req, res) => {
    try {
        const { settingType, settingKey, title, proposedChanges, currentSnapshot, justification, makerId, makerName, teamId } = req.body;
...
workspaceSettingsProposalsRouter.post('/:id/review', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const updated = await postgresRepo.reviewWorkspaceSettingProposal(id, checkerId, checkerName || 'Checker Lead', action, feedback);
...
workspaceSettingsProposalsRouter.post('/:id/escalate', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const updated = await postgresRepo.escalateWorkspaceSettingProposal(id, targetTeamId, reason, escalatedById || 'system', escalatedByName || 'Operator');
