...
export const postgresRepo = {
    ...
    async createValidationWorkflow(wf) {
        const pool = getPostgresPool();
        await pool.query(`INSERT INTO database_validation_workflows (id, name, description, target_db_id, target_table, category, stages, steps, global_success_message, global_failure_message, created_by, created_at, updated_at, is_system_default, version)
    ...
    async getCentralTransactionsByTaskId(taskId) {
        const pool = getPostgresPool();
        const { rows } = await pool.query('SELECT * FROM central_transaction_repository WHERE current_task_id = $1 OR original_task_id = $1 ORDER BY row_number ASC;', [taskId]);
        return rows.map(r => ({
            transactionKey: r.transaction_key,
            originalTaskId: r.original_task_id,
            currentTaskId: r.current_task_id,
