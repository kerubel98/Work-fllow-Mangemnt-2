...
export async function seedPostgres() {
    ...
    try {
        ...
        for (const org of INITIAL_ORGANIZATIONS) {
         ...
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (id) DO NOTHING;`, [org.id, org.name, org.domain || null, org.createdAt || new Date(), JSON.stringify(org.settings || {})]);
        }
        // 2. Users
        ...
        for (const db of INITIAL_DBS) {
            await pool.query(`INSERT INTO database_connections (id, name, type, host, port, connection_string, database_name, username, password, status, api_endpoint, created_by_admin, requires_access_approval, description, allowed_roles)
        ...
        for (const plugin of INITIAL_PLUGINS) {
            await pool.query(`INSERT INTO plugins (id, name, description, enabled, category, config)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (id) DO NOTHING;`, [
                plugin.id,
                plugin.name,
