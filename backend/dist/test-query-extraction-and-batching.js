 ...
 */
import { resolveRequiredColumnsForStage } from './services/requiredFieldResolver.js';
...
];
{
    ...
    const stage1ColNames = stage1Req.requiredColumns.map((c) => c.sourceColumn);
    ...
    const stage2ColNames = stage2Req.requiredColumns.map((c) => c.sourceColumn);
...
console.log('\n--- SUITE 4: External Data Retrieval & Correlation ---');
{
    ...
    assert(queryInfo.sql.includes('SELECT response_code, auth_time FROM dynamic_auth_feed WHERE auth_txn_id IN (?, ?) AND is_active = ?'), 'buildQueryFromExtraction builds parameterized query with selected columns, target table, and key mapping');
...
console.log('\n--- SUITE 5: Parent Issue Status Aggregation ---');
{
    ...
    assert(aggMixed.issueStatus === 'ACTION_REQUIRED', '99 CLOSED + 1 FLAGGED yields ACTION_REQUIRED (Child closure cannot close parent issue)');
