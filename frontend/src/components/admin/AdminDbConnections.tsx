...
import { 
  ...
  Table, CheckSquare, Square, ShieldCheck, FolderDown, FileSpreadsheet, Users
} from 'lucide-react';
...
export default function AdminDbConnections({
  ...
  // Team database allocation state
  ...
  useEffect(() => {
    if (teams && teams.length > 0) {
      ...
      api.getTeams().then(res => {
        if (Array.isArray(res)) setLocalTeams(res);
      }).catch(() => {});
    }
  }, [teams]);

  const [allocatingDb, setAllocatingDb] = useState<DatabaseConnection | null>(null);
  const [selectedTeamIds, setSelectedTeamIds] = useState<string[]>([]);
  const [isSavingAllocation, setIsSavingAllocation] = useState(false);
  const [allocationMessage, setAllocationMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  const handleOpenAllocationModal = (db: DatabaseConnection) => {
    setAllocatingDb(db);
    setAllocationMessage(null);
    const initialSelected = localTeams
      .filter(t => Array.isArray(t.allowedDbIds) && t.allowedDbIds.includes(db.id))
      .map(t => t.id);
    setSelectedTeamIds(initialSelected);
  };

  const handleToggleTeamAllocation = (teamId: string) => {
  ...
  const handleSaveAllocation = async () => {
    ...
    try {
      ...
      for (const t of updatedTeams) {
        ...
        const currentAllowed = Array.isArray(t.allowedDbIds) ? [...t.allowedDbIds] : [];
        const hasDb = currentAllowed.includes(allocatingDb.id);

        if (isSelected && !hasDb) {
          const next = [...currentAllowed, allocatingDb.id];
          t.allowedDbIds = next;
          await api.allocateDatabasesToTeam(t.id, next);
        } else if (!isSelected && hasDb) {
          const next = currentAllowed.filter(id => id !== allocatingDb.id);
          t.allowedDbIds = next;
          await api.allocateDatabasesToTeam(t.id, next);
        }
      }
      setLocalTeams(updatedTeams);
      setAllocationMessage({ type: 'success', text: 'Team database allocation successfully updated.' });
      setTimeout(() => {
        setAllocatingDb(null);
        setAllocationMessage(null);
      }, 1200);
    } catch (err: any) {
      setAllocationMessage({ type: 'error', text: err.message || 'Failed to update database allocation.' });
    } finally {
  ...
  return (
    <div className="space-y-6">
            ...
            >
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                ...
                <div className="flex items-center gap-1">
                  <button
                    type="button"
                    onClick={() => handleOpenAllocationModal(db)}
                    className="text-xs px-2.5 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded-lg font-semibold flex items-center gap-1 transition"
                    title="Allocate this database connection to teams"
                  >
                    <Users className="w-3 h-3" />
                    <span>Teams ({localTeams.filter(t => t.allowedDbIds?.includes(db.id)).length})</span>
                  </button>
                  <button
                    type="button"
      ...
      {/* Team Database Allocation Modal */}
      {allocatingDb && (
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full border border-slate-200 shadow-2xl p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                  ...
                  <span>Allocate Connection to Teams</span>
                </h3>
                <p className="text-xs text-slate-500 mt-0.5">
                  Database: <span className="font-semibold text-slate-800">{allocatingDb.name}</span> ({allocatingDb.databaseName || allocatingDb.type})
                </p>
              </div>
              <button
                type="button"
                onClick={() => setAllocatingDb(null)}
            ...
            <div className="bg-indigo-50/70 border border-indigo-200 rounded-xl p-3 text-xs text-indigo-900 flex items-start gap-2">
              ...
              <div>
                ...
                <p className="mt-0.5 text-indigo-800">
                  Select which teams have authorized access to this database endpoint. Members of allocated operational teams can inspect schemas and configure business column mappings on permitted tables.
                </p>
              </div>
            </div>
            ...
            {allocationMessage && (
              <div className={`p-3 rounded-xl text-xs font-semibold flex items-center gap-2 ${
                ...
                {allocationMessage.type === 'success' ? <CheckCircle className="w-4 h-4 text-emerald-600" /> : <X className="w-4 h-4 text-red-600" />}
            ...
            <div className="max-h-60 overflow-y-auto space-y-2 pr-1 divide-y divide-slate-100">
              {localTeams.length === 0 ? (
                <div className="text-center py-6 text-xs text-slate-400">
                  No teams found in system. Create teams in Team Workspace.
                </div>
              ) : (
                localTeams.map(team => {
                  const isChecked = selectedTeamIds.includes(team.id);
                  const isDbTeam = team.adminPrivileges?.canManageConnections;
                  const isOpsTeam = team.adminPrivileges?.canManageColumnMapping;

                  return (
                    <label
                      key={team.id}
                      onClick={() => handleToggleTeamAllocation(team.id)}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400">{team.description || 'No description'}</p>
                        </div>
                      </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-slate-100">
              <span className="text-xs text-slate-500 font-medium">
                {selectedTeamIds.length} of {localTeams.length} teams allocated
              </span>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                ...
                <button
                  ...
                  className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-bold transition shadow-xs disabled:opacity-50 cursor-pointer flex items-center gap-1.5"
                >
                  {isSavingAllocation ? (
                    <>
