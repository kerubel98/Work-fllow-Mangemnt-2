        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-bold text-white tracking-tight">Database Column Rules & Configurations</h2>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-500/20 text-blue-300 border border-blue-400/30">
                  Zero Hardcoded Constraints
                </span>
              </div>
              <p className="text-xs text-slate-400 mt-0.5">
                Define dynamic composite rules (Duplicate Checks, Grouping Checks, Uniqueness) with custom priority lists and real-time database preview.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <div className="flex items-center gap-1.5 bg-slate-800/90 border border-slate-700/80 rounded-lg px-2.5 py-1.5">
              ...
              <label className="text-[11px] text-slate-400 font-medium">Database:</label>
              <select
                value={selectedDbId}
            ...
            <div className="flex items-center gap-1.5 bg-slate-800/90 border border-slate-700/80 rounded-lg px-2.5 py-1.5">
              ...
              <select
                ...
                className="bg-transparent text-xs font-bold text-white focus:outline-none cursor-pointer pr-2 disabled:opacity-50 font-mono"
              >
                {tables.length === 0 ? (
                  <option value="" className="bg-slate-900 text-slate-400 font-sans">
                    {isLoadingTables ? 'Scanning Tables...' : 'No Tables Discovered'}
                  </option>
                ) : (
                  tables.map((tbl) => (
                    <option key={tbl} value={tbl} className="bg-slate-900 text-white font-mono">
                      {tbl}
                    </option>
                  ))
                )}
            ...
            <button
              ...
              className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 border border-slate-700 transition cursor-pointer disabled:opacity-50"
              title="Introspect and scan database for all tables"
            >
              <Sparkles className={`w-3.5 h-3.5 text-amber-400 ${isLoadingTables ? 'animate-spin' : ''}`} />
            ...
            <button
              ...
              className="px-3.5 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 shadow-sm transition cursor-pointer disabled:opacity-50"
            >
