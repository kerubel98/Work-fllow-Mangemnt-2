...
export interface AppNotification {
  id: string;
  userId: string; // Recipient user ID, or 'all'
  type: NotificationType;
  title: string;
  message: string;
  timestamp: string;
  isRead: boolean;
  linkTab?: string; // Tab to navigate to when clicked, e.g. 'team_workspace', 'workspace', 'direct_chat'
  targetTeamId?: string;
  targetTaskId?: string;
...
export interface Issue {
  ...
  fileMapping?: Record<string, string>; // Maps file header -> DB standard column
  firstLevelNotes?: string;
  firstLevelMappedData?: Record<string, any>[]; // Simulated parsed data
  secondLevelNotes?: string;
  solutionScript?: string;
  solutionTestResult?: string;
  solutionExecuted?: boolean;
  solutionExecutedAt?: string;
  linkedHashtag?: string; // links to HashtagPreset.tag
  chat?: ChatMessage[];
  // NEW fields for advanced features:
  assignedTechUserId?: string; // assigned tech user ID
  assignedTechUserName?: string; // assigned tech user Name
  investigationSystemId?: string; // selected environment system
  investigationEnvironment?: 'testing' | 'production'; // testing or production
  investigationTable?: string; // mapped database table
  validationStatus?: 'untested' | 'passed' | 'failed';
  validationErrors?: string[];
  queryResults?: any[]; // Simulated query results against chosen environment
  movedToTesting?: boolean; // whether operational user promoted this to testing env
  rowLabels?: Record<string, string>; // Maps row unique transaction identifier -> custom label annotation
  addedLabelColumnName?: string; // name of the custom operational labeling column
  customFilters?: { column: string; value: string }[]; // operational active dynamic criteria filters
  teamId?: string;
...
export interface HashtagPreset {
  tag: string; // e.g., "#DUPLICATE_TXN"
  category?: string;
  description: string;
  criteria: string; // Match criteria text or rules
  expectedFileStructure: string[]; // e.g. ["Txn_ID", "Card_Num", "Amount", "Status"]
  expectedHeaders?: string[];
  solutionTemplate: string; // SQL template for resolution
  author: string;
  createdAt: string;
  // NEW fields:
  fileTemplateData?: Record<string, string>[]; // mock downloadable rows
  criteriaRules?: CriteriaRule[]; // validation rules to auto-check
  workflowId?: string;
  workflowName?: string;
  processType?: SolutionProcessType;
  kpis?: HashtagKpi[];
...
export interface EnvironmentSystem {
  id: string;
  name: string; // e.g., "Front End Client", "Back End Processor"
  description: string;
  testing: EnvironmentConfig;
  production: EnvironmentConfig;
  allowedUserIds: string[]; // Specific user IDs granted privilege by admin
  allowedRoles: UserRole[]; // Roles with general access (e.g. ['operational', 'technical'])
  requireDmlApproval?: boolean; // NEW: DML/UPDATE queries require DB user (Admin) approval
}
...
export interface QueryApprovalRequest {
  ...
  requestDate: string;
  issueId?: string; // linked issue/case if any
  issueTitle?: string;
}
...
export type ExternalSystemCategory = 'CBS' | 'Switch_FE' | 'Switch_BE' | 'Settlement' | 'Gateway' | 'Ledger' | 'Custom_External';
export type BankingEnvironmentType = 'banking' | 'switching' | 'hybrid' | 'general';

export interface AppWorkingDatabaseInfo {
  engine: 'MongoDB';
  version: string;
  serverConnected: boolean;
  databaseName: string;
  desktopLocalStorageActive: boolean;
  desktopStorageEngine: 'LocalStorage / IndexedDB Cache';
...
export interface DatabaseConnection {
  ...
  // External Banking & Switching System properties
  isExternal?: boolean; // Always true for configured external databases (CBS, Switch FE/BE)
  systemCategory?: ExternalSystemCategory; // e.g. 'CBS', 'Switch_FE', 'Switch_BE'
  environmentType?: BankingEnvironmentType; // e.g. 'banking' (CBS), 'switching' (FE/BE)
  lastTestedAt?: string;
...
export interface SchemaModelFieldDef {
  key: string;
  label: string;
  description: string;
  dataType: 'string' | 'number' | 'date' | 'boolean';
...
export interface CentralRepositoryDdlLog {
  id: string;
  action: 'CREATE_TABLE' | 'ALTER_TABLE_ADD_COLUMN' | 'ALTER_TABLE_DROP_COLUMN' | 'REBUILD_TABLE' | 'INDEX_CREATE';
  columnKey?: string;
  ddlStatement: string;
  timestamp: string;
  executedBy: string;
...
export interface GlobalMappingSchemaModel {
  modelId: string;
  modelName: string;
  targetRepositoryTableName: string;
  version: string;
  description: string;
  fields: SchemaModelFieldDef[];
  primaryKey: string;
  indexes: { name: string; columns: string[]; isUnique?: boolean }[];
  sqlDdlDefinition: string;
  typeScriptDefinition: string;
  jsonSchemaDefinition: Record<string, any>;
  ormModelDefinition: string;
  lastCompiledAt: string;
  compiledBy: string;
}

export interface CentralUploadedTransactionRepositoryTable {
  tableName: string;
  dbId: string;
  dbName: string;
  ...
  createdAt: string;
  updatedAt: string;
  columns: SchemaModelFieldDef[];
  recordCount: number;
  records: Record<string, any>[];
  status: 'active' | 'rebuilding' | 'schema_updated' | 'uninitialized';
  lastDdlExecuted?: string;
  ddlHistory: CentralRepositoryDdlLog[];
}
...
export interface GlobalKeyFieldSchema {
  id: string;
  name: string;
  description?: string;
  keyFields: GlobalTransactionSchemaField[];
  createdBy: string;
  createdAt: string;
  updatedAt: string;
}
...
export interface ColumnMappingRule {
  ...
  defaultValue?: string;  // Fallback default value if blank
  transformRule?: 'none' | 'uppercase' | 'trim' | 'parse_currency' | 'mask_card';
}
...
export interface DualSourceCondition {
  ...
  comparator: 'EQUALS' | 'NOT_EQUALS' | 'CONTAINS' | 'NOT_CONTAINS' | 'NUMERIC_TOLERANCE' | 'GREATER_THAN' | 'LESS_THAN' | 'IN' | 'LOOKUP_MAP';
...
export interface TransactionExecutionSummary {
  ...
  remedySql?: string;
  intermediateReports?: Record<string, any>;
}
